...
취소는 DB 요청 플래그를 500ms 간격과 처리 경계에서 확인하는 협력적 방식입니다. 게시 직전 확인 뒤 실제 파일 쓰기 사이까지 원자적으로 막지는 못합니다. 완료한 파일/성공은 뒤늦은 취소로 지우거나 실패로 바꾸지 않습니다. 강제 종료나 DB 기록 불명 시 running/started가 남으며, 아래 검증을 거친 --resume으로만 재개합니다. `knowledge:job`은 최근 50개와 개별 이력을 읽어 이런 작업을 찾습니다. 입력은 재실행 CLI가 다시 제공하며 백그라운드 queue 소비자는 아직 없습니다. 이 기록 방식은 아래 명시적 복구 구현으로 확장했습니다. 검토 후 강제 재개와 백그라운드 Worker는 남습니다.

### 명시적 중단 복구 — 2026-09-22 구현

005 migration이 `active_attempt_id`, 30초 `lease_until`, 불변 `processing_recovery` 기록을 추가합니다. heartbeat는 만료 전 같은 시도만 연장하며 완료·호출 기록·복구 자료 쓰기는 같은 실행 소유권과 기한을 검사합니다. `--resume`은 만료된 running을 새 시도로 넘기고 이전 시도를 중단 이력으로 남깁니다. 재개도 기존 최대 시도 횟수에 포함됩니다. 시작 전 기록만 있거나 결과 없는 호출이 있으면 needs-review로 종료하며 재호출하지 않습니다.

작업 전체에는 **transaction 밖의 PostgreSQL session advisory lock**을 Vault별로 잡습니다. 모델 대기 동안 transaction/row lock은 유지하지 않습니다. 파일 잠금도 유지하며 새 형식은 hostname/PID/attempt ID/token을 완전히 기록한 뒤 hard link로 게시합니다. DB 세션 잠금을 잡은 복구자만 같은 job의 과거 attempt, 동일 hostname, PID 부재(ESRCH)를 확인해 잠금을 회수합니다. 세션 연결이 끊겨도 살아 있는 파일 소유자를 지우지 않습니다. 다른 DB/다른 기기/구형 잠금은 추측으로 회수하지 않습니다. PID가 재사용돼 살아 있으면 보수적으로 검토 대상입니다.

`plan`은 최초 날짜, `segment:N`은 검증된 구간 본문·사용량, `draft`는 완성 생성 결과입니다. 파일의 정본은 Markdown이며 DB 본문은 중단 재개용 임시 산출물입니다. 성공 transaction은 draft/segment/로그 before-image를 제거하고 문서 hash와 사용량 이력은 유지합니다. 미완료/검토 기록의 장기 정리 정책은 후속입니다. body가 같은 JSONB도 속성 순서가 바뀔 수 있어 Article의 generation_usage 출력 순서를 고정합니다.

파일 게시 전 경로·hash intent를 저장합니다. 재개 시 기존 파일 hash가 intent와 다르면 수정본을 보존하고 중단합니다. published 표식이 있는데 파일이 없어도 자동 재생성하지 않습니다. 아직 게시하지 않은 파일은 같은 입력/날짜/초안으로 만들어 배타적으로 게시합니다. 일일 로그는 이전 바이트와 추가 바이트를 기록하고 정확한 접두 일치만 이어 씁니다. append는 기존 바이트를 교체하지 않으며 이후에도 결과를 대조합니다. Article이 게시된 후의 늦은 취소는 새 생성 대신 로그 완료를 우선하되 실행 소유권 검사는 계속합니다.

이는 프로세스 강제 종료에 대한 로컬 재개 범위입니다. 전원 손실/파일시스템 손상, 모든 외부 편집 경쟁, 여러 기기 공유 Vault를 보장하지 않습니다. 구형 running(lease 없음), 사용자 편집, 과금/생성 결과 불명, 취소/시도 상한으로 종료된 작업의 남은 잠금은 검토 후 별도 조치가 필요합니다. 자동으로 DB 상태나 남은 잠금을 초기화하지 않습니다. 입력은 동일 CLI에서 다시 제공합니다.

검증은 실제 PostgreSQL과 별도 자식 프로세스의 SIGKILL을 사용합니다. 테스트 DB에서만 lease 시간을 과거로 이동해 만료를 재현합니다. Raw/API/구간/초안/Article/로그/완료 기록 경계, 사용자 편집, 동시 재개, DB 세션 손실과 살아 있는 소유자를 검사합니다. 모델 호출은 모의 응답이며 추가 API 비용은 없습니다.
...
기본 파일 잠금은 단일 로컬 명령의 중첩 쓰기를 막습니다. 선택적 --track-job은 PostgreSQL에 지속 작업 상태를 저장하며, --resume은 위 소유권·checkpoint·write-intent 검사를 거쳐 중단된 작업을 재개합니다. 백그라운드 감지/재개와 운영 Worker는 후속 구현입니다.
...
모델 업그레이드만으로 저장·복구·문서 품질 문제가 해결된다고 가정하지 않습니다. 현재 검증할 수 있는 결함부터 개선하고, 아래 표에서 구현한 범위와 후속 후보를 구분합니다.

| 상태·시점 | 해결할 문제 | 개선 방향과 확인할 근거 |
|---|---|---|
| 로컬 명시적 재개 구현 · M2.1 | 파일 잠금만으로는 작업 중단·재시도·과금 이력을 복구할 수 없음 | PostgreSQL 작업/시도/사용량, lease, 구간 checkpoint, write intent와 SIGKILL 검증 완료. 백그라운드 Worker와 다중 기기 복구는 후속 |
